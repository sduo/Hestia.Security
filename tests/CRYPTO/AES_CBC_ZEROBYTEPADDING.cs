...
namespace Hestia.Security.Tests.CRYPTO
{
    ...
    public sealed class AES_CBC_ZEROBYTEPADDING
    {
        ...
        public void Test3()
        {
            ...
            byte[] output = Security.CRYPTO.AES_CBC_ZEROBYTEPADDING_ENCRYPT(k, Encoding.UTF8.GetBytes(decrypted));
            Assert.AreEqual(encrypted, Convert.ToHexString(output));
        }
        ...
        {
            var k = new ParametersWithIV(new KeyParameter(Convert.FromHexString(key)), Convert.FromHexString(iv));
            byte[] output = Security.CRYPTO.AES_CBC_ZEROBYTEPADDING_DECRYPT(k, Convert.FromHexString(encrypted));
            Assert.AreEqual(decrypted, Encoding.UTF8.GetString(output));
        }
    }
}