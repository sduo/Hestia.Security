...
namespace Hestia.Security.Tests.AES
{
    ...
    public sealed class AES128_CBC_PKCS7
    {
        ...
        public void Test1()
        {
            byte[] output = Utility.AES128_CBC_PKCS7_ENCRYPT(Convert.FromHexString(key),Convert.FromHexString(iv),Encoding.UTF8.GetBytes(decrypted));
        ...
        public void Test2()
        {
            byte[] output = Utility.AES128_CBC_PKCS7_DECRYPT(Convert.FromHexString(key), Convert.FromHexString(iv), Convert.FromHexString(encrypted));
        ...
        public void Test3()
        {
            ...
            byte[] output = Utility.AES128_CBC_PKCS7_DECRYPT(key, iv, encrypted);
            Assert.AreEqual(decrypted, Encoding.UTF8.GetString(output));
        }        
    }
}